...
## 3. 追加可能・推奨ライブラリ（エコシステムの拡張）

オフライン環境をさらに強化するために、以下の Pure Python パッケージが追加候補として推奨されます。

```bash
# 統計可視化・テキスト処理・テンプレート・ネットワーク分析パッケージの追加例
npm run wheels:add -- janome beautifulsoup4 jinja2 networkx altair tqdm
```

| 分類 | パッケージ名 | 用途・メリット |
| :--- | :--- | :--- |
| **日本語テキスト分析** | `janome` | 辞書内蔵の Pure Python 形態素解析器。外部通信不要で日本語分かち書きやワードクラウド、頻度集計が可能。 |
| **Web/データ抽出** | `beautifulsoup4`, `soupsieve` | オフラインで保存した HTML/XML ファイルからの構造化データ抽出。 |
| **レポート自動化** | `jinja2`, `markupsafe` | データ集計結果を HTML / Markdown / ログ形式の定型レポートに自動レンダリング。 |
| **ネットワーク解析** | `networkx` | 人間関係マップ、交通網、依存関係ツリー、最短経路アルゴリズムの計算。 |
| **宣言的ベクター可視化** | `altair` | Vega-Lite 準拠の美しいインタラクティブ・ベクターグラフ描画。 |
| **進捗表示** | `tqdm` | 大量ループ処理時のプログレスバーをノートブック出力に表示。 |
